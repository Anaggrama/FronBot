        ...
        If CharIndex = CasterIndex Then
            If index = 4 Then
                If charlist(CasterIndex).Inmo Then
                    ...
                    Call WriteConsoleMsg(CasterIndex, "Te has lanzado " & Hechizo(index).name & ".", FontTypeNames.FONTTYPE_FIGHT)
        ...
        Else
            If EnTeam(CharIndex) = EnTeam(CasterIndex) Then
                If index = 4 Then
                    ...
                    If charlist(CharIndex).Bot = 0 Then _
                        Call WriteConsoleMsg(CharIndex, charlist(CasterIndex).Nombre & " te ha lanzado " & Hechizo(index).name & ".", FontTypeNames.FONTTYPE_FIGHT)
                    Call WriteUpdateCharStats(CasterIndex)
                    Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessageChatOverHead(Hechizo(index).Palabras, CasterIndex, vbCyan))
                    Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessagePlayWave(Hechizo(index).WAV, charlist(CharIndex).Pos.X, charlist(CharIndex).Pos.Y))
                    Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessageCreateFX(CharIndex, Hechizo(index).FX, 0))
                ...
                ElseIf index = 6 Then
                    If SinResu = 0 Then
                        ...
                        Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessagePlayWave(Hechizo(index).WAV, charlist(CharIndex).Pos.X, charlist(CharIndex).Pos.Y))
                        Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessageCreateFX(CharIndex, Hechizo(index).FX, 0))
                        Call ServerSendData(SendTarget.ToAll, CasterIndex, PrepareMessageCharacterChange(charlist(CharIndex).iBody, charlist(CharIndex).iHead, charlist(CharIndex).heading, _
                                            CharIndex, charlist(CharIndex).iArma, charlist(CharIndex).iEscudo, charlist(CharIndex).FxIndex, charlist(CharIndex).FX.Loops, charlist(CharIndex).iCasco))
                        Call DarPrioridadTarget(1)
                        Exit Sub
                    Else
